...
-- Tree Enums
--CREATE TYPE VARCHAR(32) AS ENUM('ACTIVE', 'FROZEN', 'DRAINING');
--CREATE TYPE VARCHAR(32) AS ENUM('LOG', 'MAP', 'PREORDERED_LOG');
--CREATE TYPE VARCHAR(32) AS ENUM('RFC6962_SHA256', 'TEST_MAP_HASHER', 'OBJECT_RFC6962_SHA256', 'CONIKS_SHA512_256', 'CONIKS_SHA256');
--CREATE TYPE VARCHAR(32) AS ENUM('SHA256');
--CREATE TYPE VARCHAR(32) AS ENUM('ECDSA', 'RSA');
...
CREATE TABLE  trees (
  ...
  hash_strategy            VARCHAR(32) NOT NULL,
...
CREATE TABLE  sequenced_leaf_data(
  tree_id                   BIGINT NOT NULL,
...
CREATE OR ALTER FUNCTION public.ignore_duplicates(tree_id bigint, leaf_identity_hash bytea, leaf_value bytea, extra_data bytea, queue_timestamp_nanos bigint)
 RETURNS bit
LANGUAGE plpgsql
AS $function$
    begin
...
CREATE OR ALTER FUNCTION public.ignore_duplicates(tree_id bigint, leaf_identity_hash bytea, merkle_leaf_hash bytea, queue_timestamp_nanos bigint)
 RETURNS bit
 LANGUAGE plpgsql
AS $function$
    begin
