...
var inclusionProofInvalidTestNodes = []storage.Node{
...
func TestInclusionProofGetsIncorrectNode(t *testing.T) {
	...
	for _, testNode := range inclusionProofInvalidTestNodes {
		const rev = 100
		r, tx := getSparseMerkleTreeReaderWithMockTX(mockCtrl, rev)
		tx.EXPECT().Commit().AnyTimes().Return(nil)
		...
		proof, err := r.InclusionProof(rev, []byte(key))
		if err == nil {
			t.Fatal("InclusionProof() should've returned an error due to incorrect node from storage layer: %v", proof)
		}
