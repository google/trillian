...
func testSparseTreeFetches(ctx context.Context, t *testing.T, vec sparseTestVector) {
	...
	// we've just created:
	tx.EXPECT().GetMerkleNodes(ctx, int64(rev), nodeIDFuncMatcher{func(ids []node.NodeID) bool {
		if len(ids) == 0 {
			return false
		}
		readMutex.Lock()
