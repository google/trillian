...
func TestSignLogRoot(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// Check that the signature is correct
		obj, err := SerializeLogRoot(test.root, trillian.LogSignatureFormat_LOG_SIG_FORMAT_V1)
		if err != nil {
...
func TestSignMapRoot(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// Check that the signature is correct
		canonical, err := SerializeMapRoot(test.root, trillian.MapSignatureFormat_MAP_SIG_FORMAT_V1)
		if err != nil {
