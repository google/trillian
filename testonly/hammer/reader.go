...
func (o *validReadOps) doGetLeaves(ctx context.Context, prng *rand.Rand, latest bool) error {
	...
	if o.prevContents.Empty() {
		glog.V(3).Infof("%d: skipping get-leaves as no data yet", o.vc.MapID)
		return errSkip{}
	}
	...
	if latest {
		leaves, err = o.vc.GetAndVerifyMapLeaves(ctx, indices)
		if err != nil {
			return fmt.Errorf("failed to GetAndVerifyMapLeaves: %v", err)
		}
	} else {
		leaves, err = o.vc.GetAndVerifyMapLeavesByRevision(ctx, contents.Rev, indices)
		if err != nil {
	...
	glog.V(2).Infof("%d: got %d leaves", o.vc.MapID, len(leaves))
	return nil
}
...
func (o *validReadOps) getSMR(ctx context.Context, prng *rand.Rand) error {
	root, err := o.vc.GetAndVerifyLatestMapRoot(ctx)
	if err != nil {
	...
	glog.V(2).Infof("%d: got SMR(time=%q, rev=%d)", o.vc.MapID, time.Unix(0, int64(root.TimestampNanos)), root.Revision)
...
func (o *validReadOps) getSMRRev(ctx context.Context, prng *rand.Rand) error {
	...
	if smrRoot == nil {
		glog.V(3).Infof("%d: skipping get-smr-rev as no earlier SMR", o.vc.MapID)
		return errSkip{}
	}
	rev := int64(smrRoot.Revision)

	root, err := o.vc.GetAndVerifyMapRootByRevision(ctx, rev)
	if err != nil {
...
func (o *validReadOps) verify(root *types.MapRootV1) error {
	...
	want, err := mapContents.RootHash(o.vc.MapID, o.vc.Hasher)
	if err != nil {
		return err
	}
