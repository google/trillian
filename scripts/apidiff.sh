...
# A script which acts as a helper to bridge golang apidiff and Github actions.
# Modified from https://github.com/pohly/klog/blob/fix-apidiff/hack/verify-apidiff.sh
...
for pkg in $(go list ./...); do
  ...
  if [[ -n "${incompatible}" ]]; then
    echo >&2 "${pkg} contains incompatible changes"
    while IFS= read -r line; do
      echo >&2 "::warning:: Incompatible change: $line"
    done <<< "${incompatible}"
    retval=0
  fi
done
...
if [[ -n "${removed}" ]]; then
  echo >&2 "Removed packages:"
    while IFS= read -r line; do
      echo >&2 "::warning:: removed package $line"
    done <<< "${removed}"
  retval=0
fi
