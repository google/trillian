...
This runs several multi-process tests:
 ...
 - A [test](integration/log_integration_test.go) that starts a Trillian server
   in Log mode, together with a signer, logs many leaves, and checks they are integrated correctly.
 - A [test](integration/ct_integration_test.go) that starts a set of Trillian
   ...
   personality servers then run tests that exercise all of the
...
Re-generating mock or protobuffer files are only needed if you're changing
...
A subdirectory, to ensure that builds use a fixed version (typically because the
...
# Trailing / in the prefix is needed.
...
A person may also perform **canonicalization** on incoming data, to
...
Note that a person may need to implement its own data store,
separate from Trillian.  In particular, if the person does not
...
following available operations:
 ...
 - `GetLeaves` returns leaf information for a specified set of key values,
   optionally as of a particular revision.  The returned leaf information also includes inclusion proof data.
...
creating a newly signed tree head as a result.

![Log components](docs/LogDesign.png)


TODO: add a description of distribution: how many instances run, how distributed etc.
...
some kind of the corresponding key:value data to a Trillian Map.
