...
func TestShouldResign(t *testing.T) {
	...
	for _, test := range tests {
		fakeTimeSource := clock.FakeTimeSource{}
...
func TestElectionRunnerRun(t *testing.T) {
	fakeTimeSource := clock.NewFakeTimeSource(time.Date(2016, 6, 28, 13, 40, 12, 45, time.UTC))
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			startTime := time.Now()
			fakeTimeSource := clock.NewFakeTimeSource(startTime)
